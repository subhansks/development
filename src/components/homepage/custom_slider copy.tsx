...
export default function CustomSlider() {
  ...
  useEffect(() => {
    if (sliderRef.current) {
      ...
      if (firstItem) {
        setItemWidth(firstItem.offsetWidth + 12); // Add 12 for gap between items
        const containerWidth = sliderRef.current.offsetWidth;
        setVisibleItems(Math.floor(containerWidth / firstItem.offsetWidth));
        setAtStart(true);
        checkPosition();
      }
    }
  ...
  const scrollNext = () => {
    if (sliderRef.current) {
      ...
      // Delay checkPosition to ensure smooth scrolling
  ...
  return (
    <div className="relative mt-3 ">
      <div
        ref={sliderRef}
        className="overflow-x-scroll flex gap-3 pl-0 "
      ...
      <button
        onClick={scrollPrev}
        disabled={atStart}
        className={`absolute left-0 z-10 bg-dealguru-blue text-white px-2 py-1 rounded-lg ${
      ...
      <button
        onClick={scrollNext}
        disabled={atEnd}
        className={`absolute right-0 z-10 bg-dealguru-blue text-white px-2 py-1 rounded-lg ${
